...
function generateLayout (width, height) {
  ...
  for (var y = 0; y < height; y++) {
    ...
    for (var x = 0; x < width; x++) {
      layout[y].push(Math.random() > 0.5 ? null : 'build/img/page-screenshot.png');
    }
  }
...
var Grid = React.createClass({displayName: "Grid",
  ...
  addTileClick: function (event) {
    ...
    newLayout[event.y][event.x] = undefined;
  ...
  render: function () {
    ...
    for (var y = 0; y < this.state.layout.length; y++) {
      for (var x = 0; x < this.state.layout[0].length; x++) {
        if (this.state.layout[y][x] || typeof this.state.layout[y][x] === 'undefined') {
          nodes.push(
